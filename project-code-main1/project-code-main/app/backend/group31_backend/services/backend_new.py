...
def populate_database():
    ...
    try:
        ...
        for country in countries_data:
            ...
            leagues = getLeaguesInCountry_wikidata(country['id'])
            print(f'getting leagues in country:{country}')
            cursor.execute("INSERT OR IGNORE INTO Countries (CountryID, name) VALUES (?, ?)", (country['id'], country['name']))
            time.sleep(1)  # Manage request rate
            leagues = getLeaguesInCountry_wikidata(country['id'])
