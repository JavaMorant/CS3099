...
def getTeamsLogoById(team_id: str) -> str:
    ...
    if wikipedia_url is None:
        return "N/A"  # Return immediately if the Wikipedia URL could not be found

    try:
        wikipedia_page = requests.get(wikipedia_url)
        if wikipedia_page.status_code == 200:
            soup = BeautifulSoup(wikipedia_page.content, 'html.parser')
            logo_element = soup.find('table', class_='infobox vcard').find('img')
            if logo_element:
...
def create_database():
    try:
        conn = sqlite3.connect("Database31HardCoded.db")
...
populate_database(List_of_leagues[17])